...
import { editRecipe, getRecipeById } from "../Service/recipes";
...
function EditRecipe() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            try {
                // Make sure the category is a flat array (important fix here)
                const formattedValues = {
                    ...values,
                    category: Array.isArray(values.category[0]) ? values.category[0] : values.category,
                };

                console.log("Sending data:", formattedValues);
                const response = await editRecipe(recipeId, formattedValues);
                if (response) {
                    successMsg("Recipe updated :)");
                    nav("/");
                }
            } catch (error) {
